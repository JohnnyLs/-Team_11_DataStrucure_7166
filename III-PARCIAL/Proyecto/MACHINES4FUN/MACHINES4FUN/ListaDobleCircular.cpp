...
void ListaDobleCircular::imprimir(void)
{
    ...
    if(!vacia()){
        ...
        do{
		    ...
		    std::cout<<"  CODIGO OPERADOR#: "<<aux->getMaquina().getOperador().getPersona().getId()<<std::endl;
        ...
        std::cout<<"NO EXISTEN MAQUINAS EN LA LISTA!!\n";
    }
}
...
void ListaDobleCircular::imprimirNodo(Nodo* nodo)
{
    ...
    std::cout<<"  CODIGO OPERADOR #: "<<nodo->getMaquina().getOperador().getPersona().getId()<<std::endl;
